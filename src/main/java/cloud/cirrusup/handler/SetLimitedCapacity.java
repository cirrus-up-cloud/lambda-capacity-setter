...
public class SetLimitedCapacity  extends AbstractCapcitySetter {
    ...
    public Void handleRequest(Void aVoid, Context context) {
        ...
        context.getLogger().log("Starting <SetLimitedCapacity::handleRequest> Lambda functions.");
        ...
        if (result.getAutoScalingGroups().size() > 0) {
            ...
            for (InstanceCreditSpecification ins : res.getInstanceCreditSpecifications()) {
                ...
                if (ins.getCpuCredits().equals(UNLIMITED)) {
                    ...
                    context.getLogger().log("Modified credit for instance: " + ins.getInstanceId() + " to standard.");
                }
            }
        }
